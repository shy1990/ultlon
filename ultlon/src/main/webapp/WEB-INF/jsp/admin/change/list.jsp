		...
		<tbody>
			<c:forEach var="item" items="${data.content }">
				<tr class="am-active">
					...
					<td class="remark"
						title="${item.content.remark.toString()} ${item.content.lastModifiedBy.username }">${item.content.remark }
						${item.content.lastModifiedBy.username }</td>
					...
					<td style="width: 200px;"><c:if
							...
							<button type="button" class="am-btn am-btn-success am-radius"
								id="agree" testvalue="${item.content.id}">同意</button>
							<button type="button" class="am-btn am-btn-danger am-radius"
								id="reject" testvalue="${item.content.id}">拒绝</button>
						</c:if></td>
				</tr>
			</c:forEach>
		</tbody>
	...
	<div class="phone_main_07">
		...
		<ul data-am-widget="pagination"
			...
			<li class="am-pagination-prev "><c:choose>
					...
					<c:otherwise>
						<a
							href="admin/change?sort=createdDate,desc&imei=${imei}&page=${data.metadata.number-1}"
							class="">上一页</a>
					</c:otherwise>
				</c:choose></li>
